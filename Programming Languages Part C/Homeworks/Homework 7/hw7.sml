...
(* CHANGE add shifts for expressions of the form Shift(deltaX, deltaY, exp *)
     | Shift of real * real * geom_exp (* shift geom_exp by d_x, d_y *)
...
(* #2 *)
      | Shift(d_x, d_y, s) =>
        ...
        | Point (x, y) => eval_prog(Point(x + d_x, y + d_y), env)
        | Line (m, b) => eval_prog(Line(m, (b + d_y) - (m * d_x)), env)
        | VerticalLine (x) => eval_prog(VerticalLine(x + d_x), env)
        | LineSegment (x1, y1, x2, y2) => eval_prog(LineSegment(x1 + d_x, y1 +
        d_y, x2 + d_x, y2 + d_y), env)
        | e => eval_prog(Shift(d_x, d_y, eval_prog(e, env)), env))
    ...
    case e of
         LineSegment (x1, y1, x2, y2) => 
            ...
            then (if real_close(y1, y2)
                  then eval_prog(Point(x1, y1), [])
                  else if y2 < y1
                  then eval_prog(LineSegment(x2, y2, x1, y1), [])
                  else eval_prog(e, []))
            else if x2 < x1
            then eval_prog(LineSegment(x2, y2, x1, y1), [])
            else eval_prog(e, [])
       | Let (s, e1, e2) => eval_prog(Let(s, preprocess_prog(e1),
       preprocess_prog(e2)), [])
       | Intersect (e1, e2) => eval_prog(Intersect(preprocess_prog(e1),
       preprocess_prog(e2)), []) 
       | Shift (d_x, d_y, s) => eval_prog(Shift(d_x, d_y, preprocess_prog(s)), [])
       | _ => eval_prog(e, [])
