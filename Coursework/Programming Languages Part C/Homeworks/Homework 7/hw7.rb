...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    epsilon = 0.00001
    if ((seg.x1 - epsilon <= @x) and (@x <= seg.x2 + epsilon)) or ((seg.x2 - epsilon <= @x) and (@x <= seg.x1 + epsilon)) and ((seg.y1 - epsilon <= @y) and (@y <= seg.y2 + epsilon)) or ((seg.y2 - epsilon <= @y) and (@y <= seg.y1 + epsilon))
      self
    else
      NoPoints.new
    end
...
class Line < GeometryValue
  ...
  def intersectLine line
    if real_close(@m, line.m)
      ...
      Point.new((@b - line.b) / (@m - line.m), (@m * ((@b - line.b) / (@m - line.m))) + @b)
    end
  end
...
class LineSegment < GeometryValue
  ...
  def intersectVerticalLine vl
    vl.intersectWithSegmentAsLineResult self
  end
...
class Let < GeometryExpression
  ...
  def eval_prog env
    @e2.eval_prog(Array.new(env.insert(0, [@s, @e1])))
  end
